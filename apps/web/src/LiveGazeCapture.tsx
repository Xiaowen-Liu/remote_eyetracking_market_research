...
export function LiveGazeCapture({ collecting, sampleIntervalMs, onCalibrated, onGazeSample }: Props) {
  ...
  function record() { if (!featureRef.current) { setMessage("Face not detected. Keep both eyes visible and try again."); return; } const next = [...samples, { feature: featureRef.current, target: targets[pointIndex] }]; setSamples(next); if (pointIndex < targets.length - 1) { setPointIndex((value) => value + 1); return; } const model = fitGazeModel(next); if (!model) { setMessage("Calibration did not fit. Try again."); return; } modelRef.current = model; const quality = calibrationError(model, next); setMessage(`Calibration accepted locally (${(quality * 100).toFixed(1)}% RMS).`); onCalibrated(quality); }

  return <div className="live-gaze-capture"><div className="camera-frame"><video ref={videoRef} muted playsInline /><span className={foundFace ? "camera-state ready" : "camera-state"}>{foundFace ? "Face landmarks detected" : "Waiting for face"}</span></div><p className="fine-print">{message}</p>{!started && <button className="primary-button" type="button" onClick={() => void start()}>Allow camera and calibrate</button>}{started && !modelRef.current && <div className="participant-calibration-board"><button type="button" className="calibration-target" style={{ left: `${targets[pointIndex][0] * 100}%`, top: `${targets[pointIndex][1] * 100}%` }} onClick={record} /><p>Point {pointIndex + 1} of 9</p><button className="secondary-button" type="button" onClick={record}>Record calibration point</button></div>}{modelRef.current && <div className="participant-live-preview"><span>{collecting ? "Collecting coordinate samples" : "Calibration ready"}</span>{gaze && <i className="gaze-cursor" style={{ left: `${gaze[0] * 100}%`, top: `${gaze[1] * 100}%` }} />}</div>}</div>;
}
