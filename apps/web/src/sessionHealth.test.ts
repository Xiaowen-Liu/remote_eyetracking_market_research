...
function artifact(overrides: Partial<CollectorArtifact> = {}): CollectorArtifact {
  return {
    ...
    gazeSamples: Array.from({ length: 80 }, (_, index) => ({
      ...
      y: 0.5,
      confidence: 0.88,
      at: `2026-01-01T00:00:0${Math.min(9, Math.floor(index / 8))}.000Z`,
    })),
    calibration: {
      attempt: 1,
...
describe("collectorSessionHealth", () => {
  it("marks a completed, calibrated, well-sampled session good", () => {
    ...
    expect(health.meanConfidence).toBeCloseTo(0.88);
  });
  ...
  it("does not present sparse or incomplete collection as valid evidence", () => {
    ...
    expect(collectorSessionHealth(artifact({ gazeSamples: [{ x: 0.5, y: 0.5 }] })).grade).toBe(
      "invalid",
    );
  });

  it("labels variable calibration as directional even with dense samples", () => {
    const health = collectorSessionHealth(
      artifact({
        calibration: {
          attempt: 1,
          observed_sample_count: 35,
          error_px: 120,
          quality_grade: "variable",
          accepted: true,
        },
      }),
    );
