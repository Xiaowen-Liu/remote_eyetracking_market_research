...
describe("participant collection journey", () => {
    ...
    apiMocks.resolveParticipantLink.mockResolvedValue({
      ...
      tasks: [
        ...
        },
        {
          ...
          prompt: "Begin checkout.",
          start_url: "https://demo.example.com/pricing",
        },
      ],
    });
    apiMocks.createParticipantSession.mockResolvedValue({
      id: "session-1",
  ...
  it("takes a consented participant through calibration, tasks, batch upload, and analysis submission", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "Checkout attention study" }),
    ).toBeInTheDocument();
  ...
  it("hands webcam studies to the full browser extension instead of the reduced web calibrator", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "Continue in the browser extension" }),
    ).toBeInTheDocument();
    expect(screen.getByText(/camera check, three-sample point calibration/i)).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Allow camera and calibrate" }),
