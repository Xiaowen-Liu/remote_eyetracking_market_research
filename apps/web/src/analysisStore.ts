...
function database() {
  return new Promise<IDBDatabase>((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(request.error ?? new Error("Unable to open the local analysis archive."));
  });
}
...
function transactionResult<T>(request: IDBRequest<T>, transaction: IDBTransaction) {
  return new Promise<T>((resolve, reject) => {
    ...
    transaction.onerror = () =>
      reject(transaction.error ?? request.error ?? new Error("Local analysis storage failed."));
    transaction.onabort = () =>
      reject(transaction.error ?? new Error("Local analysis storage was aborted."));
  });
}
...
function transactionDone(transaction: IDBTransaction) {
  return new Promise<void>((resolve, reject) => {
    ...
    transaction.onerror = () =>
      reject(transaction.error ?? new Error("Local analysis storage failed."));
    transaction.onabort = () =>
      reject(transaction.error ?? new Error("Local analysis storage was aborted."));
  });
}
...
export async function storeArtifacts(studyId: string, artifacts: CollectorArtifact[]) {
  ...
  try {
    ...
    artifacts.forEach((artifact) =>
      store.put({
        key: `${studyId}:${artifact.sessionId}`,
        studyId,
        artifact,
...
export async function deleteStoredArtifact(studyId: string, sessionId: string) {
  ...
  try {
    ...
    await transactionResult(
      transaction.objectStore(storeName).delete(`${studyId}:${sessionId}`),
      transaction,
    );
  } finally {
    db.close();
