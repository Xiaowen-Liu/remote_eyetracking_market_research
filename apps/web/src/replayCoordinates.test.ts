...
import { detectScrollSegments, inferDocumentExtent, insertReplaySnapshot, projectReplaySample } from "./replayCoordinates";
...
const artifact = parseCollectorArtifact({
  ...
  events: [],
  snapshots: [{ at: "2026-01-01T00:00:00.000Z", dataUrl: "data:image/png;base64,x", viewport: { width: 1000, height: 500 }, scroll: { x: 0, y: 0 } }],
  gazeSamples: [
    { x: .5, y: .5, viewport: { width: 1000, height: 500 }, scroll: { x: 0, y: 0 } },
    { x: .5, y: .5, viewport: { width: 1000, height: 500 }, scroll: { x: 0, y: 500 } },
  ],
...
describe("replay coordinate tools", () => {
  ...
  it("projects viewport gaze into inferred document coordinates", () => {
    ...
    expect(projectReplaySample(artifact.gazeSamples![1], "document", extent)).toMatchObject({ x: .5, y: .75 });
  });
  ...
  it("inserts and orders a researcher screenshot at the playhead", () => {
    ...
    expect(next.snapshots[1]).toMatchObject({ reason: "researcher-inserted", viewport: { width: 1000, height: 500 } });
  });
});
