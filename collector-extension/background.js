...
async function request(session, path, init = {}) {
  ...
  try {
    ...
    if (error instanceof DOMException && error.name === "AbortError")
      throw new Error("Study API request timed out");
    throw error;
  } finally {
    clearTimeout(timeout);
  }
}
async function participantRequest(session, path, init = {}) {
...
function studyState(session) {
  const completed = session?.completedTasks ?? 0;
  const next = session?.protocol?.tasks?.[completed] ?? null;
  return {
    connected: Boolean(session?.protocol),
    participantToken: session?.participantToken ?? null,
    phase: session?.phase ?? "idle",
    title: session?.protocol?.title ?? null,
    consentText: session?.protocol?.consent_text ?? null,
    tasks: session?.protocol?.tasks?.map(({ position, title }) => ({ position, title })) ?? [],
    nextTask: next ? { position: next.position, title: next.title, prompt: next.prompt } : null,
...
async function enqueueSamples(session, samples) {
  ...
  const batch = gazeBatch(
    samples,
    nextBatchSequence(session.nextSequence ?? 0, session.pendingBatches ?? []),
    crypto.randomUUID(),
  );
  session.pendingBatches = [...(session.pendingBatches ?? []), batch];
  await write(session);
  for (const pending of [...session.pendingBatches]) {
    await participantRequest(session, `/participant-sessions/${session.sessionId}/gaze-batches`, {
      method: "POST",
      body: JSON.stringify(pending),
    });
    session.pendingBatches = session.pendingBatches.filter(
      (item) => item.client_batch_id !== pending.client_batch_id,
    );
    session.nextSequence = Math.max(session.nextSequence ?? 0, pending.sequence + 1);
    await write(session);
  }
  return session;
}
async function startStudy(session) {
  ...
  const access = await request(session, `/participate/${session.participantToken}/sessions`, {
    method: "POST",
    body: JSON.stringify({
      browser_family: "Chromium extension",
      viewport_width: tab.width ?? 1280,
      viewport_height: tab.height ?? 720,
      device_pixel_ratio: 1,
    }),
  });
  await participantRequest(
    { ...session, accessToken: access.access_token },
    `/participant-sessions/${access.id}/consent`,
    {
      method: "POST",
      body: JSON.stringify({ accepted: true, consent_version: protocol.consent_version }),
    },
  );
  const next = {
    ...session,
    ...newSession({ sessionId: crypto.randomUUID(), captureSnapshots: session.captureSnapshots }),
    sessionId: access.id,
...
async function finishTask(session) {
  ...
  let next = {
    ...session,
    gazeSamples: [...(session.gazeSamples ?? []), ...(drained.samples ?? [])],
  };
  await write(next);
  next = await enqueueSamples(next, drained.samples ?? []);
  await participantRequest(
    next,
    `/participant-sessions/${next.sessionId}/task-runs/${next.taskRun.id}/complete`,
    { method: "POST", body: JSON.stringify({ outcome: "completed" }) },
  );
  next = {
    ...next,
    taskRun: null,
    completedTasks: next.completedTasks + 1,
    phase: "ready",
    lastError: null,
  };
  await write(next);
...
async function submitStudy(session) {
  ...
  const next = { ...session, phase: "submitted", submitted, lastError: null };
  await write(next);
  return next;
}
...
chrome.runtime.onMessage.addListener((message, sender, respond) => {
  operations = operations
    .then(async () => {
      ...
      if (message.type === "STUDY_STATUS") {
        respond({ ok: true, state: studyState(session) });
        return;
      }
      if (message.type === "CONNECT_STUDY") {
        const token = participantToken(message.participantLink ?? "");
        if (!token) throw new Error("Paste a valid participant link or capability token");
        const apiBase = message.apiBase?.trim() || defaultApiBase;
        const protocol = await request({ apiBase }, `/participate/${token}`);
        if (!protocol.collection_policy?.webcam_gaze_enabled)
          throw new Error("This study has not enabled experimental webcam gaze collection");
        session = {
          ...newSession({
      ...
      if (message.type === "ACCEPT_CONSENT") {
        session = await startStudy(session);
        respond({ ok: true, state: studyState(session) });
        return;
      }
      if (message.type === "START_STUDY_TASK") {
        if (session.phase !== "ready")
          throw new Error("Finish accepted calibration before starting a task");
        session = await startTask(session);
        respond({ ok: true, state: studyState(session) });
        return;
      }
      if (message.type === "COMPLETE_STUDY_TASK") {
        session = await finishTask(session);
        respond({ ok: true, state: studyState(session) });
        return;
      }
      if (message.type === "SUBMIT_STUDY") {
        session = await submitStudy(session);
        respond({ ok: true, state: studyState(session) });
        return;
      }
      if (message.type === "DOWNLOAD_ARTIFACT") {
        respond({ ok: true, state: studyState(session), artifact: exportArtifact(session) });
        return;
      }
      ...
      if (message.type === "CALIBRATION_COMPLETED") {
        const calibration = {
          ...message.calibration,
          attempt: (session.calibration?.attempt ?? 0) + 1,
        };
        const recorded = await participantRequest(
          session,
          `/participant-sessions/${session.sessionId}/calibrations`,
          { method: "POST", body: JSON.stringify(calibrationPayload(calibration)) },
        );
        session = {
          ...session,
          calibration: recorded,
          phase: recorded.accepted ? "ready" : "calibrating",
          lastError: null,
        };
        await write(session);
        if (!recorded.accepted && sender.tab?.id)
          await chrome.tabs.sendMessage(sender.tab.id, { type: "COLLECTOR_RETRY_CALIBRATION" });
        respond({ ok: true, accepted: recorded.accepted, state: studyState(session) });
        return;
      }
      if (message.type === "PAGE_EVENT") {
        ...
        if (
          ...
          next = addSnapshot(next, {
            ...
            reason: message.event.type,
            viewport: message.event.detail?.viewport,
            scroll: message.event.detail?.scroll,
          });
        }
        await write(next);
        respond({ ok: true });
    ...
    .catch(async (error) => {
      ...
      if (session)
        await write({
          ...session,
          lastError: error instanceof Error ? error.message : "Extension operation failed",
        });
      respond({
        ok: false,
        error: error instanceof Error ? error.message : "Extension operation failed",
        state: studyState(await read()),
      });
    });
  return true;
});
