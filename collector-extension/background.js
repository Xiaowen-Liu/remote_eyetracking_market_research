...
import { apiUrl, calibrationPayload, defaultApiBase, gazeBatch, nextBatchSequence, participantToken, resolveStudyUrl } from "./core/study-session.js";
...
async function startTask(session) {
  ...
  if (tab.url !== taskUrl) await navigateAndMessage(tab.id, taskUrl, { type: "COLLECTOR_START_TASK", taskTitle: task.title }); else await chrome.tabs.sendMessage(tab.id, { type: "COLLECTOR_START_TASK", taskTitle: task.title }); return next;
}
...
chrome.runtime.onMessage.addListener((message, sender, respond) => {
  operations = operations.then(async () => {
    ...
    if (message.type === "DOWNLOAD_ARTIFACT") { respond({ ok: true, state: studyState(session), artifact: exportArtifact(session) }); return; }
