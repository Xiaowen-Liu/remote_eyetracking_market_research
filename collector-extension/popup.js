...
function render(next) {
  state = next;
  $("#connect-panel").hidden = Boolean(state?.connected);
  $("#study-panel").hidden = !state?.connected;
  if (!state?.connected) {
    showStatus(state?.error, Boolean(state?.error));
    return;
  }
  $("#study-title").textContent = state.title;
  $("#progress").textContent = `Completed tasks: ${state.completedTasks} of ${state.tasks.length}`;
  $("#consent-text").textContent = state.consentText;
  hidePanels();
  if (state.phase === "consent") $("#consent-panel").hidden = false;
  if (state.phase === "calibrating") $("#calibration-panel").hidden = false;
  if (state.phase === "ready" && state.nextTask) {
    $("#task-title").textContent = `Task ${state.nextTask.position}: ${state.nextTask.title}`;
    $("#task-prompt").textContent = state.nextTask.prompt;
...
$("#consent-check").onchange = (event) => {
  $("#accept-consent").disabled = !event.target.checked;
};
...
$("#start-task").onclick = async () => {
  try {
    await invoke("START_STUDY_TASK");
    window.close();
  } catch (error) {
    showStatus(error.message, true);
  }
};
$("#complete-task").onclick = async () => {
  try {
    await invoke("COMPLETE_STUDY_TASK");
  } catch (error) {
    showStatus(error.message, true);
  }
};
$("#submit-study").onclick = async () => {
  try {
    await invoke("SUBMIT_STUDY");
  } catch (error) {
    showStatus(error.message, true);
  }
};
$("#download").onclick = async () => {
  try {
    const { artifact } = await invoke("DOWNLOAD_ARTIFACT");
    const url = URL.createObjectURL(
      new Blob([JSON.stringify(artifact, null, 2)], { type: "application/json" }),
    );
    await chrome.downloads.download({
      url,
      filename: "webgaze-extension-artifact.json",
      saveAs: true,
    });
  } catch (error) {
    showStatus(error.message, true);
