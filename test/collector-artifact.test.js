...
test("collector artifact excludes camera video and gates snapshots by consented policy", () => {
  ...
  const eventful = addEvent(base, {
    type: "scroll-settled",
    url: "https://example.test",
    at: "2026-01-01T00:00:00Z",
  });
  const withoutSnapshot = addSnapshot(eventful, {
    url: "https://example.test",
  ...
  assert.deepEqual(exportArtifact(withoutSnapshot).privacy, {
    rawCameraVideo: false,
    eventCollection: true,
    visibleTabSnapshots: false,
  });
});
...
test("collector snapshots retain viewport context when the policy is enabled", () => {
  ...
  const captured = addSnapshot(base, {
    url: "https://example.test",
    dataUrl: "data:image/jpeg;base64,x",
    reason: "page-open",
    viewport: { width: 1280, height: 720 },
