...
import {
  ...
  observeSegment,
  pointerSpeed,
} from "../collector-extension/core/boundary-calibration.js";
...
test("requires slow movement and accepts near or passed arrivals", () => {
  ...
  assert.equal(
    hasArrived(observeSegment({ x: 0, y: 0 }, { x: 100, y: 0 }, { x: 96, y: 30 })),
    true,
  );
  assert.equal(
    hasArrived(observeSegment({ x: 0, y: 0 }, { x: 100, y: 0 }, { x: 105, y: 50 })),
    true,
  );
  assert.equal(
    hasArrived(observeSegment({ x: 0, y: 0 }, { x: 100, y: 0 }, { x: 70, y: 80 })),
    false,
  );
});
